...
export const BookCheckoutPage = () => {
  ...
  const bookId = (window.location.pathname).split("/")[2]; // we will get the localhost:3000/checkout/<bookID>, will be bookId
  ...
  useEffect(() => {
    ...
    fetchBooks().catch((error: any) => { // if error occurs

      setIsLoading(false);
      setHttpError(error);
    })

  }, [isCheckedOut]); // all the code goes inside []
  ...
  useEffect(() => {
    const fetchUserCurrentLoansCount = async () => {
      if (authState && authState.isAuthenticated) {
        ...
        const currenLoansCountResponseJson = await currentLoansCountResponse.json();
        setCurrentLoansCount(currenLoansCountResponseJson);
      }
